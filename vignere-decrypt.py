...
for i in range(len(text)):
   ...
   if ord(text[i]) >= 65 and ord(text[i]) <= 90:
     #upper!
     sys.stdout.write(chr( ((ord(text[i]) - ord('A')) - curkey + 26) % 26 + ord('A')))
     # update curkey
    ...
    #problem with program, writes outa full word using one of the commands
    #Need to do letter by letter change key and implment in for loop